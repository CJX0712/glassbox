...
但重排的结果**不会直接成为最终顺序**。它作为第三路排序，以一半权重与第一阶段的
RRF 顺序再融合一次：

```
最终顺序 = RRF([一阶段融合顺序, 重排顺序], weights=[1.0, 0.5])
```

这一层「降权再融合」来自一次实测翻车：只要把重排顺序直接当作最终顺序，一个在查询
语言上力不从心的交叉编码器就会**无人制衡地把正确答案往下压**——详见
[重排权重：一条反直觉的曲线](#重排权重一条反直觉的曲线)。降权融合后，同一批中文
查询从 11/12 升到 12/12，而重排本身的排序依然在证据链里完整留痕。
...
5. **对抗性重排无法推翻融合榜首** —— 自检注入一个把候选名单**完全倒序**打分的
   恶意交叉编码器，断言一阶段融合的冠军仍然排第一。这是上面那次重排翻车的回归
   保护：只要 `GLASSBOX_RERANK_WEIGHT < 1`，`1/(k+1) + w/(k+n) > 1/(k+n) + w/(k+1)`
   在算术上恒成立，所以这条断言**不会偶发失败**，同时它又能精确捕捉「把最终顺序
   直接设成重排顺序」这个改法。
...
| **重排只作信号，不作否决** | 交叉编码器可能在查询语言上力不从心；降权再融合后，坏重排无法单方面推翻好召回（实测 11/12 → 12/12） |
| **重排模型选轻量的那个** | 实测 1.11 GB 的多语言模型并没换来更好的排序（11/12），0.15 GB 的反而 12/12——体积大不等于判得准 |
...
python -m glassbox selftest                            # 41/41
```
...
语料是仓库自带的那 12 篇中英双语文档，查询是 12 条中文问句（每条对应一篇文档），
指标是 **top-1 命中率**：

| 最终排序策略 | top-1 | top-3 |
|---|---|---|
| 纯 RRF（完全不用重排） | 11/12 | 12/12 |
| 重排顺序**直接接管**最终排序（原实现） | **10/12** | 12/12 |
| 二次融合 · 重排权重 0.25 | 11/12 | 12/12 |
| 二次融合 · 重排权重 **0.5**（现默认） | **12/12** | 12/12 |
| 二次融合 · 重排权重 0.75 | 12/12 | 12/12 |
| 二次融合 · 重排权重 1.0（等权） | 11/12 | 12/12 |

两点值得留意：

1. **「重排接管最终排序」比干脆不用重排还差。** 默认重排器
   `jina-reranker-v1-turbo-en` 是英文模型，面对中文查询时会把 BM25 与稠密检索
   **都排第一**的正确段落压下去。召回从来没问题（top-3 恒为 12/12），坏的是
   **排序**——而重排当时没有任何制衡。
2. **权重曲线不是单调的**：0.5 / 0.75 最好，推到 1.0 反而退化。对一个可信度不足的
   交叉编码器，**温和地听它，比全听它更好**。

同一实验里把重排器换成 1.11 GB 的 `jina-reranker-v2-base-multilingual`（多语言版），
`w=0.5` 下仍是 11/12 —— **大 7 倍的模型并没有换来更好的排序**，所以默认仍留在
0.15 GB 那个上。

```bash
python -m glassbox retrieve "RRF 是怎么把两路结果融合的"   # 对比阶段名次与最终名次
```
...
SSE 真的在流、引用真的带回来了、41 条不变量在服务进程里也全绿。任何一条断了都返回非零退出码。
